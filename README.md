...
### 3.6 排障：首次初始化 / 登录返回 500（PBKDF2 迭代上限）

托管版 Cloudflare Workers 的 WebCrypto 对 PBKDF2 迭代次数有 **100,000** 的硬上限（[workerd#1346](https://github.com/cloudflare/workerd/issues/1346)），超过该上限时 `deriveBits` 在线上会失败。旧代码使用 150,000 次默认迭代，首次 `/setup`、`/login` 或改密可能返回 `internal_error`（HTTP 500），Worker 日志中可能出现类似：

```
Pbkdf2 failed: iteration counts above 100000 are not supported
```

本地 workerd / Miniflare 不强制执行这个上限，所以该问题在本地冒烟测试中不会复现。当前代码已把默认迭代次数改为 100,000。

处理方式：

1. 用包含修复的代码重新部署**同一个** Worker（`npm run deploy`）；如果使用 Deploy to Cloudflare 按钮创建的 Git 仓库，push 后触发重新部署即可。不需要删除 D1 数据库、清空表或重跑迁移。
2. **不要**为这个错误再次点击 Deploy to Cloudflare 按钮：按钮会创建新的 D1 数据库与新的 Worker 项目，旧数据不会自动迁移。
3. 初始化失败发生在写入管理员记录之前，因此修复部署后直接重新打开 `/setup` 完成初始化即可。若短时间内反复尝试，可能触发初始化限流（每 IP 15 分钟 10 次），等待限流窗口结束后再试。
4. 例外情况：如果数据库是**本地**用旧默认值（150,000 次迭代）创建并已写入管理员哈希后再迁到托管环境，旧哈希在线上校验时也会超限失败。此时按第 7 节的「重置管理员」删除管理员记录后重新初始化即可；这属于迁移个例，与普通首次初始化失败无关。
...
| 主题 | 做法 |
| --- | --- |
| 口令 | WebCrypto PBKDF2-SHA256，16 字节随机盐，默认 100,000 次迭代（托管版 Cloudflare Workers 的 PBKDF2 硬上限，超过会在线上初始化/登录失败，详见 3.6；本地 workerd 不强制执行该上限）；哈希串自带算法与迭代次数，登录时可自动升级旧参数（升级写操作带哈希条件守卫，绝不会覆盖并发修改后的新密码） |
...
集成测试直接调用与线上相同的 `functions/_lib/router.ts`（Workers 与 Pages 共用），并使用真实的 D1（含 `CHECK`/外键约束与事务语义），覆盖：初始化只能一次、并发登录/初始化/改密限流、会话哈希存储、被动读取不续期而显式活动续期、登出与改密撤销（含当前设备）、同源/协议/端口校验、请求体上限、未知字段与危险协议拒绝、分组迁移与级联删除、跨分组排序的原子性与陈旧版本回滚、无变更请求的陈旧版本拒绝、600+ 条链接的批量导入 / 恢复 / 排序在 SQL 预算内完成、300 分组 / 5000 链接的备份往返、接近内容预算边界的多字节 Unicode 导出/恢复往返与超预算回滚、超过 10000 的安全整数排序位置往返、非法恢复、书签导入去重、生产默认 PBKDF2 迭代次数在模拟的托管 100000 上限内完成初始化 / 登录 / 未知用户派生 / 改密，以及 Worker 适配层（精确 `/api` 与 `/api/*` 的 no-store JSON 404、未登录 401、非 API 请求委派给 `ASSETS`）。
